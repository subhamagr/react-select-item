...
export default class SelectItem extends React.Component {
  ...
  render() {
    let className = 'react-select-item-container';
    if (this.props.className) {
      className += ' ' + this.props.className
    }
    if (this.isMultiple()) {
      className += ' react-select-item-multi';
    }
    if (!this.hasValue()) {
      className += ' react-select-item-empty'
    }
  ...
  renderNativeSelect = () => {
    ...
    return (
      <div className="react-select-item-native">
  ...
  renderOption = (option, i) => {
    let className = 'react-select-item-option';
    if (i === this.state.focusedIndex) {
      className += ' react-select-item-option-focused'
    }
    if (this.isSelected(option.value)) {
      className += ' react-select-item-option-selected'
    }
    if (option.disabled) {
      className += ' react-select-item-option-disabled'
    }
    const aProps = {
      id: this.state.id + '-' + i,
      onClick: option.disabled ? (e) => interceptEvent(e) : this.handleChange(option.value),
  ...
  renderCloseButton = () => {
    if (this.isMultiple() && this.props.closeText) {
      const btnProps = {
        ...
        className: 'react-select-item-close',
        onBlur: this.handleBlur,
        onFocus: this.handleFocus
      };
